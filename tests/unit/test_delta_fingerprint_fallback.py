...
def test_delta_fingerprint_extracts_hist_and_hash(monkeypatch, tmp_path):
    ...
    assert hist.get("ΔIMPORT", 0) >= 1 or hist.get("DELTA_IMPORT", 0) >= 1
    assert hist.get("MODIFY_SIG", 0) >= 1 or hist.get("CHANGE_SIG", 0) >= 1
    assert isinstance(data.get("hash"), str) and len(data["hash"]) >= 8
...
def test_doclint_ok_and_failure(monkeypatch, tmp_path):
    ...
    for name in required:
        _write_current_doc(docs / name, f"T {name}")
    ...
    _write_current_doc(docs / "92_playbooks.md", "T 92")
