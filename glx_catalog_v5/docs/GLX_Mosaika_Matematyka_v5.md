...
- **Δ** — wektor kroku, Δ=(ΔS,ΔH,ΔZ).
- **α,β** — udziały: α=S/(S+H), β=H/(S+H) (jeśli S+H>0).
...
> AST to szczególny przypadek: \\(V=V_{\text{AST}}, E=E_S \cup E_H\\) z **Z** wynikającym z głębokości bloku.
...
- \\(D=\mathbb{Z}_{\ge 0}\times \mathbb{Z}_{\ge 0}\times \mathbb{Z}\\), elementy to \\( \Delta=(\Delta S,\Delta H,\Delta Z)\\),
...
W Mozaice ΔZ jest legalne **wyłącznie** przez `bucket_shift` lub zmianę współczynnika abstrakcji \\(\lambda\\).
...
\\[ \text{insert}(o\,|\,a\!\prec\!x_2):=
   ...
   \end{cases} \quad
   \Rightarrow\quad \Delta S \gets \Delta S + 1,\ \Delta H \text{ wg typów połączeń z } a/x_2. \\]
...
tak, by otrzymać minimalne \\(\Delta\\) i **niezerwanie** spójności H‑mostów.
...
**Naturalność (sketch):** dla morfizmu \\(f:a\to b\\) w \\(\mathcal{A}\\) istnieje \\(\eta\\) taka, że
\\( \Phi(f) \circ \eta_a = \eta_b \circ \Psi(\Phi(f)) \\). W praktyce: transformacje „shadow/plan”
są spójne — brak „teleportacji” semantyki między warstwami.

---

## 5) Metryki i pseudo‑odległość
- \\( \alpha=\frac{S}{S+H} \,,\ \beta=\frac{H}{S+H} \,,\ S+H>0 \\); przy \\(S+H=0\\) odkładamy \\((\alpha,\beta)=(0,0)\\).
- **Głębokość**: \\(Z\\) oraz poziom bucket(n) (skwantowany).
- **Dystans** (pseudo‑metryka) dla jednostek \\(X,Y\\):
\\[ d(X,Y) = w_S\,|\alpha_X-\alpha_Y| + w_H\,|\beta_X-\beta_Y| + w_Z\,|Z_X-Z_Y|, \\]
...
| `bucket_jump(b→b')` | (0,0,b'−b) | (0,0,b'−b) | zmiana poziomu |
| `reassign(node,K→K')` | (+1,+h,0) | (+1,+h,0) | migracja do innego centroidu |
...
\\( \mathcal{J} = \lambda_H \sum \beta + \lambda_Z \sum |ΔZ| + \lambda_S \sum \text{rozproszenie}(S) \\).
...
- **I4 (monotoniczność celu):** \\(\mathcal{J}_{\text{po}} \le \mathcal{J}_{\text{przed}}\\).
...
- *Producenci `image.result` o β>β\***: wybiera pliki/centroidy z dominującym H.
- *Naruszenia I2*: brak dopasowania ścieżki domknięcia wyzwala fallback‑plan.

---

## 9) BUS jako algebra procesów (monada błędu)
BUS implementuje kompozycję efektów z monadą błędu \\(\mathsf{E}\\). Kompozycja zdarzeń \\(m\bind f\\) propaguje
`fail-closed`. **Fallback** to morfizm \\( \kappa: \mathsf{E}X \to \mathsf{E}X \\) wybierający plan naprawczy
na podstawie \\(\Delta\\) i ścieżki \\(\pi\\) naruszenia — preferując minimalne \\(\Delta H\\) i lokalne \\(\Delta S\\).
...
\\[ \min_{\text{akcje}} \ d_\Phi(\text{AST},\text{MZ}) \quad \text{s.t.}\quad I_1\wedge I_2\wedge I_3\wedge I_4. \\]
...
Stąd suma ΔZ po ścieżce kończy się różnicą głębokości. W Mozaice ΔZ pojawia się **tylko** w `bucket_jump`
lub przez zmianę \\(\lambda\\) (reguły sterujące). Walidator I1 odrzuca inne źródła ΔZ.
...
- **Widoki EGDB** → α/β/δ/ε per plik/kafelek/temat; naruszenia I1–I4.
