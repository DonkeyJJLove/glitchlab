...
> **Werdykt:** w swojej klasie (hybryda AST⇄cechy przestrzenne) to **bardzo dobry algorytm praktyczny**:
...
1. **Domyślnie używaj Φ2 + Δ≈0.5 + λ≈0.0** (to wynika z Pareto).
2. **Kalibracja na danych realnych**: strojenie `thr` i `τ` (soft-labels) + sanity-check CR\_TO.
3. **Opcjonalne ulepszenie**: uczony selektor Φ (np. mały klasyfikator na meta-wektorach) — zachowując D\_M i inwarianty.
4. **Monitoring produkcyjny**: loguj ΔJ i Align w czasie; alarmuj przy spadku Align o >0.05 wzgl. baseline.
