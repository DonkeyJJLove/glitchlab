# glitchlab/gui/views/menu.py
# -*- coding: utf-8 -*-
from __future__ import annotations

import sys
import tkinter as tk
from tkinter import filedialog, messagebox
from typing import Any, Dict, List, Optional


class MenuBar:
    """Tk menubar publikujący zdarzenia (bez logiki domenowej)."""

    def __init__(self, master: tk.Misc, *, bus: Optional[Any] = None, show_algorithms: bool = False) -> None:
        self.master = master
        self.bus = bus
        self.show_algorithms = bool(show_algorithms)

        self._menubar = tk.Menu(master)
        self._file = tk.Menu(self._menubar, tearoff=False)
        self._recent = tk.Menu(self._file, tearoff=False)
        self._view = tk.Menu(self._menubar, tearoff=False)
        self._run = tk.Menu(self._menubar, tearoff=False)
        self._help = tk.Menu(self._menubar, tearoff=False)

        self._recent_files: List[str] = []

        self._build()
        self.master.configure(menu=self._menubar)
        self._bind_accels()

    # ---- public API ----
    def set_bus(self, bus: Any) -> None:
        self.bus = bus

    def set_recent_files(self, paths: List[str]) -> None:
        self._recent_files = list(paths or [])
        self._rebuild_recent()

    # ---- build ----
    def _build(self) -> None:
        # File
        self._menubar.add_cascade(label="File", menu=self._file)
        self._file.add_command(label=self._lab("Open…", "Ctrl+O"), command=self._open_image)
        self._file.add_cascade(label="Open Recent", menu=self._recent)
        self._file.add_separator()
        self._file.add_command(label=self._lab("Save As…", "Ctrl+S"), command=self._save_image)
        self._file.add_separator()
        # UWAGA: presety emitują *_request aby obsłużył to PresetService
        self._file.add_command(label=self._lab("Open Preset…", "Ctrl+P"), command=self._open_preset_request)
        self._file.add_command(label="Save Preset As…", command=self._save_preset_request)
        self._file.add_separator()
        self._file.add_command(label=self._lab("Quit", "Ctrl+Q"), command=self._quit)

        # View
        self._menubar.add_cascade(label="View", menu=self._view)
        self._view.add_command(label="Toggle HUD (F10)", command=lambda: self._emit("ui.view.toggle_hud", {}))
        self._view.add_command(label="Toggle Left Tools (F8)", command=lambda: self._emit("ui.view.toggle_left", {}))
        self._view.add_command(label="Toggle Right Panel (F9)", command=lambda: self._emit("ui.view.toggle_right", {}))
        self._view.add_separator()
        self._view.add_command(label="Fullscreen", command=lambda: self._emit("ui.view.fullscreen", {}))

        # Run
        self._menubar.add_cascade(label="Run", menu=self._run)
        self._run.add_command(label=self._lab("Apply Filter", "Enter"), command=lambda: self._emit("ui.run.apply_filter", {}))
        self._run.add_command(label="Apply Preset", command=lambda: self._emit("ui.run.apply_preset", {}))
        self._run.add_separator()
        self._run.add_command(label="Cancel", command=lambda: self._emit("ui.run.cancel", {}))

        # Help
        self._menubar.add_cascade(label="Help", menu=self._help)
        self._help.add_command(label="About…", command=self._about)

        self._rebuild_recent()

    # ---- helpers ----
    def _about(self) -> None:
        try:
            messagebox.showinfo("About GlitchLab", "GlitchLab – GUI v3 (refactor)")
        except Exception:
            self._emit("ui.about", {})

    def _rebuild_recent(self) -> None:
        self._recent.delete(0, "end")
        if not self._recent_files:
            self._recent.add_command(label="(empty)", state="disabled")
            return
        for p in self._recent_files[:12]:
            self._recent.add_command(
                label=self._short_path(p),
                command=lambda path=p: self._emit("ui.files.open", {"path": path}),
            )
        self._recent.add_separator()
        self._recent.add_command(label="Clear List", command=lambda: self._emit("ui.files.clear_recent", {}))

    # ---- commands ----
    def _open_image(self) -> None:
        fn = filedialog.askopenfilename(
            title="Open Image",
            filetypes=[
                ("Images", "*.png;*.jpg;*.jpeg;*.bmp;*.webp;*.tif;*.tiff"),
                ("All files", "*.*"),
            ],
        )
        if fn:
            self._emit("ui.files.open", {"path": fn})

    def _save_image(self) -> None:
        fn = filedialog.asksaveasfilename(
            title="Save Image As",
            defaultextension=".png",
            filetypes=[
                ("PNG", "*.png"),
                ("JPEG", "*.jpg;*.jpeg"),
                ("WEBP", "*.webp"),
                ("BMP", "*.bmp"),
                ("TIFF", "*.tif;*.tiff"),
            ],
        )
        if fn:
            self._emit("ui.files.save", {"path": fn})

    # Presety – żądania do PresetService (on_ui=True)
    def _open_preset_request(self) -> None:
        self._emit("ui.presets.open_request", {})

    def _save_preset_request(self) -> None:
        # Jeżeli masz edytor presetów, on powinien opublikować aktualny tekst;
        # tutaj tylko prosimy serwis o dialog i zapis (on poda ścieżkę).
        self._emit("ui.presets.save_request", {"text": ""})

    def _quit(self) -> None:
        self._emit("ui.app.quit", {})
        try:
            self.master.quit()
        except Exception:
            pass

    # ---- low-level ----
    def _emit(self, topic: str, payload: Dict[str, Any]) -> None:
        if self.bus is not None and hasattr(self.bus, "publish"):
            try:
                self.bus.publish(topic, dict(payload))
                return
            except Exception:
                pass
        try:
            messagebox.showinfo("Event", f"{topic}\n{payload}")
        except Exception:
            pass

    def _bind_accels(self) -> None:
        is_mac = (sys.platform == "darwin")
        mod = "Command" if is_mac else "Control"
        b = self.master.bind_all
        b(f"<{mod}-o>", lambda e: (self._prevent(e), self._open_image()))
        b(f"<{mod}-s>", lambda e: (self._prevent(e), self._save_image()))
        b(f"<{mod}-p>", lambda e: (self._prevent(e), self._open_preset_request()))
        b(f"<{mod}-q>", lambda e: (self._prevent(e), self._quit()))
        b("<F8>", lambda e: (self._prevent(e), self._emit("ui.view.toggle_left", {})))
        b("<F9>", lambda e: (self._prevent(e), self._emit("ui.view.toggle_right", {})))
        b("<F10>", lambda e: (self._prevent(e), self._emit("ui.view.toggle_hud", {})))
        b("<Return>", lambda e: (self._prevent(e), self._emit("ui.run.apply_filter", {})))
        # (opcjonalnie) F11 dla fullscreen:
        # b("<F11>", lambda e: (self._prevent(e), self._emit("ui.view.fullscreen", {})))

    @staticmethod
    def _lab(label: str, accel: str) -> str:
        return f"{label}\t{accel}"

    @staticmethod
    def _short_path(path: str, maxlen: int = 60) -> str:
        return path if len(path) <= maxlen else f"{path[:maxlen//2-2]}…{path[-maxlen//2+2:]}"

    @staticmethod
    def _prevent(event: tk.Event) -> None:
        try:
            event.widget.focus_set()
        except Exception:
            pass
