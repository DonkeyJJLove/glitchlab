# glitchlab/gui/generic_from_panel.py
# -*- coding: utf-8 -*-
...
class GenericFormPanel(BasicPanel):
    """
    Uniwersalny panel wyciągający pola z ctx.defaults oraz ctx.params.
    Wszystkie klucze zawierające 'mask' pokazuje jako Combobox z listy masek.
    """
    ...
    def _build(self):
        ttk.Label(self, text=self.ctx.filter_name if self.ctx else "Filter",
                  font=("TkDefaultFont", 10, "bold")).pack(anchor="w", padx=8, pady=(6, 2))
        inner = ttk.Frame(self);
        inner.pack(fill="both", expand=True, padx=8, pady=(0, 8))

        params = dict(self.ctx.defaults) if self.ctx else {}
        ...
        for i, (key, val) in enumerate(params.items()):
            ...
            if "mask" in key.lower():
                names = self.ctx.mask_names() if self.ctx else ["(none)", "full"]
                ...
                chk.grid(row=0, column=1, sticky="w", padx=6)
                self.vars[key] = var
