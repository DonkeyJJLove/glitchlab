...
class PixelSortAdaptivePanel(ttk.Frame):
    ...
    Daje kontrolę nad kierunkiem, triggerem, progiem, długością segmentu, kluczem i odwróceniem.
    Zawiera presety oraz szybkie ustawienia maski.
    """
    def __init__(self, master: tk.Misc, ctx: Optional[PanelContext] = None, **kw: Any) -> None:
        ...
        p0 = {**DEFAULTS, **(self.ctx.defaults or {}), **(self.ctx.params or {})}
        ...
        self.var_mask_key    = tk.StringVar(value=str(p0.get("mask_key", "")))
    ...
    def _build_ui(self) -> None:
        ...
        row1 = ttk.LabelFrame(self, text="Tryb", padding=8); row1.pack(fill="x", padx=8, pady=4)
        ...
        ttk.Entry(row2, textvariable=self.var_mask_key, width=14).grid(row=1, column=1, sticky="w", padx=6, pady=(6,0))
        ttk.Button(row2, text="edge", command=lambda: self._set_mask("edge")).grid(row=1, column=2, sticky="w", pady=(6,0))
        ttk.Button(row2, text="clear", command=lambda: self._set_mask("")).grid(row=1, column=3, sticky="w", pady=(6,0))
        row2.columnconfigure(3, weight=1)

        row3 = ttk.LabelFrame(self, text="Segmenty", padding=8); row3.pack(fill="x", padx=8, pady=4)
