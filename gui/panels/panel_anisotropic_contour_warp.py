...
class ACWPanel(ttk.Frame):
    """
    Panel sterowania dla anisotropic_contour_warp.
    Emituje params przez ctx.on_change(params) przy każdej zmianie.
    """
    def __init__(self, master: tk.Misc, ctx: Optional[PanelContext] = None, **kw: Any) -> None:
        super().__init__(master, **kw)
        self.ctx = ctx or PanelContext(filter_name="anisotropic_contour_warp",
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Scale(r1, from_=0.0, to=5.0, variable=self.var_strength).grid(row=0, column=1, sticky="ew", padx=6)
        ...
        ttk.Spinbox(r1, from_=1, to=12, textvariable=self.var_iters, width=6).grid(row=1, column=1, sticky="w", padx=6, pady=(6,0))
        ...
        ttk.Combobox(r2, values=("3","5"), textvariable=self.var_ksize, state="readonly", width=6)\
        ...
        ttk.Scale(r3, from_=-2.0, to=2.0, variable=self.var_edge_bias).grid(row=0, column=1, sticky="ew", padx=6)
        ...
        ttk.Entry(r3, textvariable=self.var_mask_key, width=18).grid(row=1, column=1, columnspan=2, sticky="ew", padx=6, pady=(6,0))
        ...
        ttk.Scale(r4, from_=0.0, to=2.0, variable=self.var_use_amp).grid(row=0, column=1, sticky="ew", padx=6)
        ttk.Entry(r4, textvariable=self.var_use_amp, width=6).grid(row=0, column=2, sticky="w")
        ttk.Checkbutton(r4, text="clamp", variable=self.var_clamp).grid(row=1, column=0, sticky="w", pady=(6,0))

        # Quick presets
        qp = ttk.Frame(self, padding=(8,0)); qp.pack(fill="x", pady=(0,6))
        ttk.Button(qp, text="Soft flow", command=lambda: self._apply_preset(0.6, 3, 1, 0.0, 0.0))\
            .pack(side="left", padx=2)
        ttk.Button(qp, text="Edge flow", command=lambda: self._apply_preset(1.5, 3, 2, 0.0, 0.8))\
            .pack(side="left", padx=2)
        ttk.Button(qp, text="Smoothed", command=lambda: self._apply_preset(1.0, 5, 2, 1.0, 0.3))\
            .pack(side="left", padx=2)

    def _bind_all(self) -> None:
        for v in (self.var_strength, self.var_ksize, self.var_iters, self.var_smooth,
