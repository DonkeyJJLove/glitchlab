...
class SpectralShaperPanel(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Spinbox(row2, from_=-180.0, to=180.0, increment=1.0, textvariable=self.var_angle, width=8).grid(row=0, column=1, sticky="w", padx=6)
        ttk.Label(row2, text="ang_width").grid(row=0, column=2, sticky="w")
        ttk.Spinbox(row2, from_=1.0, to=180.0, increment=1.0, textvariable=self.var_angw, width=8).grid(row=0, column=3, sticky="w", padx=6)
        ...
        row4 = ttk.LabelFrame(self, text="Maska przestrzenna", padding=8); row4.pack(fill="x", padx=8, pady=4)
