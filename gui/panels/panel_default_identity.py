...
class DefaultIdentityPanel(ttk.Frame):
    MODES = ("identity","gray","edges","edges_overlay","mask_overlay","amp_overlay","r","g","b")

    def __init__(self, master: tk.Misc, ctx: Optional[PanelContext] = None, **kw: Any) -> None:
        super().__init__(master, **kw)
        self.ctx = ctx or PanelContext(filter_name="default_identity", defaults={}, params={}, on_change=None, cache_ref={})
        ...
        self.var_edge_ksize = tk.IntVar(value=int(_v("edge_ksize", 3)))
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Combobox(top, values=self.MODES, textvariable=self.var_mode, state="readonly", width=18)\
            .grid(row=0,column=1,sticky="w",padx=6)

        # strength + clamp
        row1 = ttk.Frame(self, padding=(8,2)); row1.pack(fill="x")
        ttk.Label(row1, text="strength").pack(side="left")
        ttk.Scale(row1, from_=0.0, to=2.0, variable=self.var_strength).pack(side="left", fill="x", expand=True, padx=6)
        ...
        ttk.Combobox(eg, values=("3","5"), textvariable=self.var_edge_ksize, state="readonly", width=6)\
            .grid(row=0, column=1, sticky="w", padx=6)
        ...
        ttk.Label(am, text="mask_key").grid(row=1, column=0, sticky="w")
        ttk.Entry(am, textvariable=self.var_mask_key, width=16).grid(row=1, column=1, columnspan=2, sticky="ew", padx=6)
