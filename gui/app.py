...
# ---------------------------------------------------------------------------
# Modular widgets and services (v3 refactor)
#
# Attempt to import ParamForm, Ruler, Toolbox, EventBus and PipelineRunner
# from the modular packages.  If these modules are not present (e.g. when
# running the legacy monolithic GUI), the fallback definitions later in
# this file will be used.  Assigning None prevents NameError.
try:
    from glitchlab.gui.widgets.param_form import ParamForm  # type: ignore
    from glitchlab.gui.widgets.ruler import Ruler  # type: ignore
    from glitchlab.gui.widgets.toolbox import Toolbox as _Toolbox
except Exception:
    ParamForm = None  # type: ignore
    Ruler = None  # type: ignore
    _Toolbox = None  # type: ignore

# Capture imported classes under distinct names before any local fallback
ParamFormImported = ParamForm if 'ParamForm' in locals() else None
_RulerImported = Ruler if 'Ruler' in locals() else None
_ToolboxImported = _Toolbox if '_Toolbox' in locals() else None

try:
    from glitchlab.gui.event_bus import EventBus  # type: ignore
    from glitchlab.gui.services.pipeline_runner import PipelineRunner  # type: ignore
except Exception:
...
class _Ruler(ttk.Frame):
    """Prosty ruler bez przekazywania nieznanych opcji do tk.Canvas."""
...
# ----------------- TOOLBOX (fallback zgodny) -----------------
class _ToolboxFallback(ttk.Frame):
    """Fallback używany, gdy widgets.toolbox.Toolbox nie jest dostępny."""
    def __init__(self, parent, *, on_mode_change=None, on_toggle=None, **kwargs):
        super().__init__(parent)
        self._on_mode_change = on_mode_change
        self._on_toggle = on_toggle
        # Stan dla zgodności z app.py:
        self.var_cross = tk.BooleanVar(value=False)
        self.var_rulers = tk.BooleanVar(value=True)
        ttk.Label(self, text="Toolbox (fallback) — widgets/toolbox.py niezaładowany").pack(padx=6, pady=6, anchor="w")
    ...
    def set_enabled(self, flag: bool):
        if callable(self._on_toggle):
            try:
                self._on_toggle("enabled", bool(flag))
            except TypeError:
                self._on_toggle(bool(flag))

    def set_cross(self, v: bool):
        self.var_cross.set(bool(v))
        if callable(self._on_toggle):
            try:
                self._on_toggle("cross", bool(v))
            except TypeError:
                self._on_toggle(bool(v))

    def set_rulers(self, v: bool):
        self.var_rulers.set(bool(v))
        if callable(self._on_toggle):
            try:
                self._on_toggle("rulers", bool(v))
            except TypeError:
                self._on_toggle(bool(v))


# After defining the local fallback widgets, override them with imported versions (if available).
try:
    # Use the imported ParamForm if available
    if 'ParamFormImported' in globals() and ParamFormImported is not None:
        ParamForm = ParamFormImported  # type: ignore
    # Use the imported Ruler under the alias `_Ruler` if available, else fallback
    if '_RulerImported' in globals() and _RulerImported is not None:
        _Ruler = _RulerImported  # type: ignore
    else:
        _Ruler = _Ruler  # keep fallback
    # Use the imported Toolbox under the alias `_Toolbox` if available, else fallback
    if '_ToolboxImported' in globals() and _ToolboxImported is not None:
        _Toolbox = _ToolboxImported  # type: ignore
    else:
        _Toolbox = _ToolboxFallback  # fallback
except Exception:
    # Silently ignore any issues during override; fallback definitions remain in effect
    pass


# ------- główna aplikacja -------
def _mk_slider(parent, label, var, vmin, vmax, row):
...
class App(ttk.Frame):
    ...
    def _build_layout(self, root: tk.Tk):
        ...
        self.toolbox = _Toolbox(
            toolbar,
            on_mode_change=self._on_tool_mode_change,   # legacy / alias on_tool_changed
            on_toggle=self._on_tool_toggle,             # legacy (name, enabled)
        )
    ...
    def _build_left_tools(self, p: ttk.Frame):
        ...
        _mk_slider(p, "Roll (Z°)", self.var_roll, -180, 180, row=3)
        _mk_slider(p, "Pitch (°)", self.var_pitch, -45, 45, row=4)
        _mk_slider(p, "Yaw (°)",   self.var_yaw,   -45, 45, row=5)
    ...
    def _on_tool_toggle(self, key: str, state: bool):
        if key == "cross":
            ...
            # nic specjalnego – akceptuj dla zgodności
            pass
    ...
    def _apply_view_overlays(self):
        ...
        try:
            ...
            pass
        self._update_crosshair_visibility()
        if self.toolbox and hasattr(self.toolbox, "set_rulers") and hasattr(self.toolbox, "set_cross"):
            try:
