...
logger = logging.getLogger("glitchlab.app.panel_loader")
...
def _probe_panels_registry() -> Dict[str, Dict[str, str]]:
    """
    Próbuje odczytać rejestr paneli z glitchlab.app.panels.__init__ (jeśli dostępny).
    Dozwolony format:
        PANELS = {
    ...
    """
    try:
        mod = importlib.import_module("glitchlab.app.panels")
...
def _discover_panel_keys_fallback() -> Dict[str, Dict[str, str]]:
    ...
    Gdy brak rejestru, wykryj moduły paneli po nazwie pliku:
        glitchlab.app.panels.panel_<key>
    """
    out: Dict[str, Dict[str, str]] = {}
    try:
        pkg = importlib.import_module("glitchlab.app.panels")
...
def list_available_panels() -> list[PanelSpec]:
    ...
    for key, meta in sorted(reg.items()):
        ...
        module = f"glitchlab.app.panels.panel_{key}"
...
def _import_panel_class(spec: PanelSpec) -> Optional[Type]:
    ...
    Import klasy panelu wg specyfikacji. Obsługuje też starą konwencję modułu:
      glitchlab.app.panels.<key>_panel
    Zwraca None przy błędzie.
    """
    candidates = [
        (spec.module, spec.class_name),                                   # nowa konwencja
        (f"glitchlab.app.panels.{spec.key}_panel", spec.class_name),      # stara konwencja
    ]
...
def _construct_fallback(parent: tk.Misc, bus: Any, on_apply: Callable[[dict], None], key: str):
    ...
    try:
        mod = importlib.import_module("glitchlab.app.generic_form_panel")
...
def get_panel_class(filter_name: str) -> Optional[Type]:
    ...
    spec = PanelSpec(
        key=filter_name,
        title=filter_name,
        module=f"glitchlab.app.panels.panel_{filter_name}",
