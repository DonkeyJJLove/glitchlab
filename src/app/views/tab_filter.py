...
try:
    from glitchlab.app.panel_loader import instantiate_panel  # type: ignore
except Exception:
...
# --- ParamForm (fallback) ----------------------------------------------------
try:
    from glitchlab.app.widgets.param_form import ParamForm  # type: ignore
except Exception:
...
class TabFilter(ttk.Frame):
    ...
    def _module_candidates(self, name: str) -> List[str]:
        ...
        return [
            f"glitchlab.app.panels.panel_{canon}",
            f"glitchlab.app.panels.{canon}_panel",
        ]
    ...
    def _scan_panels_dir(self) -> List[str]:
        ...
        Szukamy:
          - glitchlab.app.panels.panel_<name>.py  → <name>
          - glitchlab.app.panels.<name>_panel.py  → <name>
        """
        names: set[str] = set()
        try:
            pkg = importlib.import_module("glitchlab.app.panels")
        except Exception as ex:
            self._log(f"scan panels: import 'glitchlab.app.panels' failed → {ex}", "WARN")
            return []
        try:
